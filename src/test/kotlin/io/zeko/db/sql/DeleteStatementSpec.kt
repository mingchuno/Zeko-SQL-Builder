    ...
    Spek({
        ...
        describe("Query Builder Delete Statement test") {
            context("Given a set of Entities User, UserRole and Address") {
                ...
                it(
                    "should match delete parameterized sql when passing entity with properties set"
                ) {
                    val user =
                        User().apply {
                            name = "O'Connor"
                ...
                it("should match delete sql using where conditions") {
                    val sql =
                        Delete(User())
                            .where("id" greater 100, "age" less 80, "name" eq "Leng")
                            .toSql()
                    debug(sql)
                ...
                it("should match delete sql using where by passing Query object") {
                    val sql =
                        Delete(User())
                            .where(Query().where("id" greater 100, "name" eq "Leng"))
                            .toSql()
                    debug(sql)
                ...
                it(
                    "should match delete sql using where Query if given entity properties & Query at the same time"
                ) {
                    val sql =
                        Delete(
                                User().apply {
                            ...
                            .where(Query().where("id" greater 100))
                            .toSql()
                    debug(sql)
                ...
                it("should match delete sql when using entity class name as table") {
                    val sql = Delete(Address().apply { user_id = 1 }).toSql()
                    debug(sql)

...
class QueryCustomExpressionSpec :
    Spek({
        fun debug(msg: Any) {
        ...
        describe("Query Builder Custom Expression test") {
            context("Given a query extension forUpdate()") {
                it("should match sql with Select...For Update") {
                    ...
                    debug(sql)
                    assertEquals(
                        "SELECT * FROM user WHERE name = ? LIMIT 1 OFFSET 0 FOR UPDATE",
                        sql
                    )
                }
            }
            ...
            context("Given a query extension union()") {
                it("should match sql with Select...Union Select...") {
                    fun Query.union(query: Query): Query {
                        this.addExpressionAfter(
                            CustomPart.WHERE,
                            QueryBlock("UNION (", query.toSql(), ")")
                        )
                        return this
                    }
                    ...
                    val sql =
                        Query()
                            ...
                            .from("user")
                            .where("name" eq "Leng")
                            .union(Query().fields("id", "first_name").from("customer"))
                            .order("first_name")
                            .toSql()
                    debug(sql)
                    assertEquals(
            ...
            context("Given multiple query extension") {
                it("should match sql with Select Distinct...Union Select...") {
                    ...
                    fun Query.unionAll(query: Query): Query {
                        this.addExpressionAfter(
                            CustomPart.WHERE,
                            QueryBlock("UNION ALL (", query.toSql(), ")")
                        )
                        return this
                    }
                    ...
                    val sql =
                        Query()
                            ...
                            .from("user")
                            .where("name" eq "Leng")
                            .union(Query().distinct().fields("id", "first_name").from("customer"))
                            .unionAll(
                                Query()

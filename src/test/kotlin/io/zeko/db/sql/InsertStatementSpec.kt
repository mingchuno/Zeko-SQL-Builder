        ...
        describe("Query Builder Insert Statement test") {
            context("Given a set of Entities User, UserRole and Address") {
                it("should match insert sql when passing entity with properties set with Map") {
                    val sql = Insert(User(mapOf("id" to 1, "name" to "O'Connor"))).toSql()
                    debug(sql)
                    assertEquals("INSERT INTO users ( id, name ) VALUES ( 1, 'O''Connor' )", sql)
                }

                it(
                    "should match insert parameterized sql when passing entity with properties set with Map"
                ) {
                    val sql = Insert(User(mapOf("id" to 1, "name" to "O'Connor")), true).toSql()
                    debug(sql)
                    assertEquals("INSERT INTO users ( id, name ) VALUES ( ?, ? )", sql)
                }

                it("should match insert sql when passing pairs") {
                    val sql = Insert(User("id" to 1, "name" to "O'Connor")).toSql()
                    debug(sql)
                    assertEquals("INSERT INTO users ( id, name ) VALUES ( 1, 'O''Connor' )", sql)
                }

                it(
                    "should match insert sql for column part only when not passing in any properties"
                ) {
                ...
                it("should match insert sql when using escape table name") {
                    val sql = Insert(User("id" to 1, "name" to "Leng")).escapeTable(true).toSql()
                    debug(sql)
                ...
                it("should match insert sql when using entity class name as table") {
                    ...
                    debug(sql)
                    assertEquals(
                        "INSERT INTO address ( user_id, street1, street2 ) VALUES ( 1, 'Jalan 123', 'Taman OUG' )",
                        sql
                    )
                }
                ...
                it(
                    "should match insert...select and on duplicate key update sql when passing Query object"
                ) {
                    val sql =
                        Insert(User(), "id", "name")
                            .select(
                    ...
                    debug(sql)
                    assertEquals(
                        "INSERT INTO users ( id, name ) SELECT user_id, fullname FROM customer WHERE status = 0 ON DUPLICATE KEY UPDATE repeated = repeated + 1, is_modified = true",
                        sql
                    )
                }
                ...
                it("should match insert...on duplicate key update sql when passing Query object") {
                    ...
                    debug(sql)
                    assertEquals(
                        "INSERT INTO users ( id, name, age ) VALUES ( 3, 'Joey Tan', 43 )  ON DUPLICATE KEY UPDATE repeated = repeated + 1, is_modified = true",
                        sql
                    )
                }

                it(
                    "should match insert...on duplicate key update parameterized sql when passing Query object"
                ) {
                    val user =
                        User().apply {
                            id = 3
                    ...
                    debug(sql)
                    assertEquals(
                        "INSERT INTO users ( id, name, age ) VALUES ( ?, ?, ? )  ON DUPLICATE KEY UPDATE repeated = repeated + ?, is_modified = ?",
                        sql
                    )
                }

                it("should match insert ignore sql when ignore() is used") {
                    val sql = Insert(User("id" to 1, "name" to "Leng")).ignore().toSql()
                    debug(sql)

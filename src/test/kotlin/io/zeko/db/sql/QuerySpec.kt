        ...
        describe("Query Builder DSL test") {
            context("Given one table select statement") {
                ...
                it("should match sql with fields specified and order by") {
                    val sql = Query().fields("id", "name", "age").from("user").order("name").toSql()
                    debug(sql)
                ...
                it("should match sql with fields specified and order by two fields") {
                    val sql =
                        Query().fields("id", "name", "age").from("user").order("id", "name").toSql()
                    debug(sql)
                    assertEquals("SELECT id, name, age FROM user ORDER BY id ASC, name ASC", sql)
                }

                it(
                    "should match sql with fields specified and order by two fields different directions"
                ) {
                    val sql =
                        Query()
                            .fields("id", "name", "age")
                ...
                it(
                    "should match sql with where condition field value IN a list(non-parameterized)"
                ) {
                    val sql =
                        Query()
                            .fields("id", "name", "age")
                ...
                it(
                    "should match sql with where condition field value IN a string list (non-parameterized)"
                ) {
                    val sql =
                        Query()
                            .fields("id", "name", "age")
                    ...
                    debug(sql)
                    assertEquals(
                        "SELECT id, name, age FROM user WHERE name IN ('Leng','Bat''s Man')",
                        sql
                    )
                }
                ...
                it("should match sql multiple where conditions with raw string values") {
                    val sql =
                        Query()
                            .fields("id", "name", "age", "nickname")
                            .from("user")
                            .where(
                                eq("name", "nickname", true),
                                ("id" greater 1),
                                sub(like("name", "%bat", true) or like("name", "man%", true))
                            )
                            .toSql()
                    debug(sql)
                    assertEquals(
                ...
                it("should match sql with between where conditions of numbers") {
                    val sql =
                        Query()
                            .fields("id", "name", "age")
                            .from("user")
                            .where("name" eq "Bat Man" and ("age" between (10 to 50)))
                            .toSql()
                    debug(sql)
                    assertEquals(
                        "SELECT id, name, age FROM user WHERE name = ? AND age BETWEEN 10 AND 50",
                        sql
                    )
                }
                ...
                it("should match sql with fulltext search(MySQL)") {
                    val sql = Query().fields("*").from("user").where("name" match "smit").toSql()
                    debug(sql)
                    assertEquals(

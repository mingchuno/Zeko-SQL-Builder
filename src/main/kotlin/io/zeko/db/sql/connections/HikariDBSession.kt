...
open class HikariDBSession : DBSession {
    ...
    private fun PreparedStatement.setParam(idx: Int, v: Any?) {
        if (v == null) {
            ...
            when (v) {
                ...
                // when doing Entity prop type mapping with datetime_utc, it will be auto converted
                // to ZonedDateTime with value in DB consider as UTC value
                is ZonedDateTime -> {
                ...
                is OffsetDateTime ->
                    this.setTimestamp(idx, Timestamp(Date.from(v.toInstant()).time))
                is Instant -> this.setTimestamp(idx, Timestamp(Date.from(v).time))
                is java.time.LocalDateTime ->
                    this.setTimestamp(
                        idx,
                        Timestamp(LocalDateTime.parse(v.toString()).toDate().time)
                    )
                is LocalDate ->
                    this.setDate(
                        idx,
                ...
                is LocalTime ->
                    this.setTime(
                        idx,
                        java.sql.Time(
                            org.joda.time.LocalTime.parse(v.toString()).toDateTimeToday().millis
                        )
                    )
                ...
                is org.joda.time.LocalTime ->
                    this.setTime(idx, java.sql.Time(v.toDateTimeToday().millis))
    ...
    protected fun prepareStatement(
        ...
        val stmt: PreparedStatement =
            if (mode > -1) rawConn.prepareStatement(sql, mode) else rawConn.prepareStatement(sql)
    ...
    private fun <T> resultSetToObjects(
        rs: ResultSet,
        dataClassHandler: (dataMap: Map<String, Any?>) -> T
    ): List<T> {
        val md = rs.metaData

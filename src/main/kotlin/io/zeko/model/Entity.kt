...
public abstract class Entity {
            ...
            when (type) {
                ...
                Type.DATETIME -> {
                    if (value !is String) {
                        ...
                        // Vertx JDBC client returns date time field as String and already converted
                        // to UTC
                        val pattern =
                            if (
                                value.indexOf("Z") == value.length - 1 &&
    ...
    protected fun convertZoneDateTime(value: Any, useSystem: Boolean = false): ZonedDateTime {
        if (value !is String) {
            ...
            if (dateStr.indexOf("T") > 0) {
                patternStr =
                    if (dateStr.indexOf(".") > 0) "yyyy-MM-dd'T'HH:mm:ss.${repeatMs(dateStr)}XXX"
                    else "yyyy-MM-dd'T'HH:mm:ssXXX"
            } else {
                // Apache ignite returns "2020-05-06 17:15:03.322Z" for timestamp columns
                patternStr =
                    if (dateStr.indexOf(".") > 0) "yyyy-MM-dd HH:mm:ss.${repeatMs(dateStr)}XXX"
                    else "yyyy-MM-dd HH:mm:ssXXX"
            }
        ...
        val pattern =
            if (value.indexOf("Z") == value.length - 1 && value.indexOf(".") > 0) {
        ...
        val systemZoneDateTime =
            ZonedDateTime.parse(value, pattern).withZoneSameInstant(ZoneId.systemDefault())
    ...
    override fun toString(): String {
        ...
        dataMap().entries.forEach { str += "${it.key}-> ${it.value}, " }
